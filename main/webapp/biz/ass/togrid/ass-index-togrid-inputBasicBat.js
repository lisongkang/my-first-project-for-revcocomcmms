...
AssIndexTogridInputBasicBat=({
	init : function(){
		 $(".form-biz-ass-togrid-ass-index-togrid-inputBasicBat").data("formOptions", {
		        bindEvents : function() {
		            var $form = $(this);
		            
		            $form.find("button[name='btn_togridBat']").click(function(){
		            	if (!AssIndexTogridInputBasicBat.checkRequiredInput($form)){
	            		    return false;
	            	    }
		    			...
		    			var url = WEB_ROOT + '/biz/ass/togrid/ass-index-togrid!doSaveBat';
		 ...
		 AssIndexTogridInputBasicBat.initModes();
		 AssIndexTogridInputBasicBat.initGrids();
		 AssIndexTogridInputBasicBat.initStores();
	}, 
	
	checkRequiredInput : function ($form) {
		var text = "";
		var afterText = "";
		if(AssIndexTogridInputBasicBat.checkNull($form.find('#tgridid').val()))
			text += "所属网格、";
		if(AssIndexTogridInputBasicBat.checkNull($form.find('#assid').val()))
			text += "考核内容、";
		if(AssIndexTogridInputBasicBat.checkNull($form.find('#mode').val()))
			text += "完成目标方式、";
		if(AssIndexTogridInputBasicBat.checkNull($form.find('#bdate').val()))
			text += "考核起始日、";
	...
	initModes : function() {
		var $form = $(".form-biz-ass-togrid-ass-index-togrid-inputBasicBat");
	...
	initGrids : function() {
		...
		var url = WEB_ROOT + "/biz/ass/togrid/ass-index-togrid!getGrids";
...
$(function() {
	AssIndexTogridInputBasicBat.init();
});