		...
		callback : {
			...
			onCheck : function(event, treeId, treeNode) {
				...
				if (treeNode.gtype == 0) {
					...
					AssIndexMonprogressIndex.initDataFromTree(grids);
				}
...
//		$.getJSON($this.url, function(result) {
		$("#monGridTree").ajaxJsonUrl($this.url, function(result) {
			if (result.type == "success" ){
				$.fn.zTree.init($("#monGridTree"), $this.setting, result.userdata);
				$this.ztreeObj = $.fn.zTree.getZTreeObj("monGridTree");
