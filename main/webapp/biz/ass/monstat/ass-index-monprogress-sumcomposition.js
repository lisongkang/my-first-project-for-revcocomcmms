		...
		callback : {
			...
			onCheck : function(event, treeId, treeNode) {
				...
				if (treeNode.gtype == 0) {
					...
					AssIndexMonprogressSumIndex.initDataFromTree(grids);
				}
...
//		$.getJSON($this.url, function(result) {
		$("#sumMonGridTree").ajaxJsonUrl($this.url, function(result) {			
			if (result.type == "success" ){
				$.fn.zTree.init($("#sumMonGridTree"), $this.setting, result.userdata);
				$this.ztreeObj = $.fn.zTree.getZTreeObj("sumMonGridTree");
