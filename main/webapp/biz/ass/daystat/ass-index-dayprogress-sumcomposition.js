var SumDayGridTreeObj = {
	ztreeObj : {},
//	url : WEB_ROOT + "/market/grid-info!gridTreeForManagerGrid",
		...
		callback : {
			...
			onCheck : function(event, treeId, treeNode) {
				...
				if (treeNode.gtype == 0) {
					...
					AssIndexDayprogressSumIndex.initDataFromTree(grids);
				}
...
//		$.getJSON($this.url, function(result) {
		$("#sumDayGridTree").ajaxJsonUrl($this.url, function(result) {				
			if (result.type == "success" ){
				$.fn.zTree.init($("#sumDayGridTree"), $this.setting, result.userdata);
				$this.ztreeObj = $.fn.zTree.getZTreeObj("sumDayGridTree");
			} else {
