...
import com.maywide.core.exception.ExceptionLogger;
import com.maywide.core.web.json.HibernateAwareObjectMapper;
...
public class Jackson2LibHandler implements ContentTypeHandler {
	...
	@Override
	public String fromObject(ActionInvocation arg0, Object obj, String resultCode, Writer stream) throws IOException {
		  ObjectMapper mapper = HibernateAwareObjectMapper.getInstance();
	        if (obj instanceof Throwable) {
	            HttpServletRequest request = ServletActionContext.getRequest();
	            String msg = ExceptionLogger.logForHttpRequest((Throwable) obj, request);
	...
	@Override
	public void toObject(ActionInvocation arg0, Reader in, Object target) throws IOException {
		ObjectReader or = HibernateAwareObjectMapper.getInstance().readerForUpdating(target);
        or.readValue(in); //, new TypeReference<clazz>);
