...
public class CmpHttpClientImpl {

	private static Logger log = LoggerFactory.getLogger(CmpHttpClientImpl.class);
	...
	private static void copyBaseRespInfo2callLog(String respJsonStr, RemotecallLog remotecallLog) {
		...
		CmpHttpBaseRespBO cmpBaseResp = getCmpHttpBaseResp(respJsonStr);
		if (null != cmpBaseResp) {
			if (StringUtils.isNotBlank(cmpBaseResp.getErrcode())) {
				remotecallLog.setRetcode(cmpBaseResp.getErrcode());
			}

			if (StringUtils.isNotBlank(cmpBaseResp.getResult())) {
				remotecallLog.setRetmsg(cmpBaseResp.getResult());
			}
		}
	}

	private static CmpHttpBaseRespBO getCmpHttpBaseResp(String jsonStr) {
		CmpHttpBaseRespBO baseResp = null;
		if (StringUtils.isBlank(jsonStr)) {
			return null;
		}
		try {
			baseResp = (CmpHttpBaseRespBO) BeanUtil.jsonToObject(jsonStr, CmpHttpBaseRespBO.class);
		} catch (Exception e) {
			e.getStackTrace();
		}
		return baseResp;
