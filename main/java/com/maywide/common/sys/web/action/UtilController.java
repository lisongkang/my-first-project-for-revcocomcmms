package com.maywide.common.sys.web.action;
...
public class UtilController extends SimpleController {
    ...
    @MetaData(value = "补发对账文件")
    public HttpHeaders reissueFile() throws Exception {
        HttpServletRequest request = ServletActionContext.getRequest();
        String reissueDateStr = request.getParameter("reissueDate");
        Date reissueDate = DateUtils.parseDate(reissueDateStr);
        
        unifyPayBillService.uploadBillFile(reissueDate);
       
        model = OperationResult.buildSuccessResult("补发文件操作成功");
        return new DefaultHttpHeaders().disableCaching();
    }
